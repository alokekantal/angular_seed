...
import { jobLibraryCreationService } from './jobLibraryCreation.service'
...
import { ApplicationComponemtListService } from '../applicationComponemtList/applicationComponemtList.service';

import * as _ from 'lodash';

@Component({
  selector: 'job-library-creation',
  templateUrl: './jobLibraryCreation.component.html',
  styleUrls: ['./jobLibraryCreation.component.scss'],
  animations: [routerTransition()],
  providers: [jobLibraryCreationService,ApplicationComponemtListService, UtilityService, CommonService]
})
export class JobLibraryCreationComponent implements OnInit {  
  checkbocCollapse = {  
      isCollapsedToBeInclude: true,
      isCollapsedMeterials: true,
      isCollapsedTheWorkToBeSurvedAlsoBy: true
  };
  BREADCRUMB: any = AppConstant.BREADCRUMB_JOB_LIBRARY_CREATION;
  jobForm: FormGroup;
  ...
  constructor(private translate: TranslateService,
    ...
    private jobLibraryCreationService: jobLibraryCreationService) {
  ...
  saveJob() {
    ...
    this.jobLibraryCreationService.saveOrUpdateJob(this.jobDataForSave).subscribe(res => {
  ...
  ngOnInit() {
    ...
    if (this.id == 0) {
      ...
      PromiseList.push(this.jobLibraryCreationService.loadJobDetails(this.id));
    }
